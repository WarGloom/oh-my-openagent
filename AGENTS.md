...
- **Runtime-fallback watchdog:** `packages/omo-opencode/src/hooks/runtime-fallback/first-prompt-watchdog.ts` detects no-progress subagent sessions, starts from the 90s first-prompt window, and feeds progress-aware runtime fallback timers (`first_progress_timeout_seconds`, `stall_timeout_seconds`, `hard_timeout_seconds`) so long model thinking after progress is tolerated while true hangs still trigger fallback / abort.
