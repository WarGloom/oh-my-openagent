...
If the Codex pass is launched as a raw CLI subprocess, it is your responsibility to keep a live wait attached to it. Launch with a recorded PID and output file, then immediately run a bounded foreground waiter that checks the output for a terminal first-line verdict (`VERDICT: OKAY` or `VERDICT: CHANGES REQUIRED`) and checks whether the PID exited. Dual-review ordering matters: a native Momus completion/mailbox update is not evidence the raw Codex pass is done. If Momus finishes first and Codex is still running, immediately enter the bounded Codex waiter; do not stop at a status note that Codex is still running. A bounded timeout only means the current wait window expired: if the PID is still alive and no verdict exists, run another bounded wait instead of stopping. If the PID exits without a verdict, inspect the output and treat it as `CHANGES REQUIRED` / failed review. Do not use unbounded followers such as `tail -f`, and do not say "I'll wait" unless an actual tool call is actively waiting or polling for the verdict.
