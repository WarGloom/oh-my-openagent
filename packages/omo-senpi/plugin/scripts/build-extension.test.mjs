...
describe("checkExtensionCurrent", () => {
  ...
  test("#given current generated outputs #when ensured #then exact bundle bytes are preserved", async () => {
    // given
    const outputs = await sharedOutputs()
    const before = await Promise.all([
      readFile(outputs.outputPath),
      readFile(outputs.taskOutputPath),
      readFile(outputs.memberOutputPath),
      readFile(outputs.memoryMcpOutputPath),
      readFile(outputs.supervisorOutputPath),
      readFile(outputs.advisorRuntimeOutputPath),
    ])

    // when
    const result = await ensureExtensionCurrent(outputs)
    ...
    // then
    expect(result).toMatchObject({ ok: true, rebuilt: false })
