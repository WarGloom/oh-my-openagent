...
function buildSubagentTypeSchema(availableSubagentNames?: readonly string[]) {
  ...
  const rawSubagentNames = availableSubagentNames && availableSubagentNames.length > 0
    ? availableSubagentNames
    : BUILTIN_SUBAGENT_TYPES.map((subagent) => subagent.name)
  const subagentNames = rawSubagentNames.filter((name) => !isPlanAgent(name) && !isDirectSubagentTypeDisabled(name))
