...
describe("createDelegateTask schema", () => {
	...
	test("#given available subagent names omitted #when tool is created #then plan is excluded from the fallback schema", () => {
		//#given
		const toolDefinition = createDelegateTask({ manager: {} as never, client: {} as never, directory: "/tmp/test" })
		...
		expect(subagentSchema.def.innerType.def.entries).toHaveProperty("metis", "metis")
	})
