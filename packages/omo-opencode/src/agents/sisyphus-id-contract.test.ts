...
describe("Sisyphus background task ID guidance", () => {
  ...
  for (const [name, buildPrompt] of promptBuilders) {
    test(`#given ${name} prompt #when describing background tasks #then bg ids and session ids are disambiguated`, () => {
      ...
      expect(prompt).toMatch(/all-complete|all sibling background tasks complete|every sibling background task has finished/)
      expect(prompt).toMatch(/partial (?:reminder|notification)/)
      ...
      expect(prompt).not.toContain("On receiving `<system-reminder>` → collect results")
    })
  }
})
