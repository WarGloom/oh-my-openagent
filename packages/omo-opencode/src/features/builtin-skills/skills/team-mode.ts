...
Do **not** create a team for independent read-only searches, one-off audits, or parallel fact gathering where the workers do not need to talk to each other. Use parallel \`task(..., run_in_background=true, load_skills=[])\` subagents for that. Team mode is for shared task state, peer messaging, lifecycle tracking, or worktree/tmux coordination.
...
For inline \`team_create({ inline_spec })\` calls, prefer the runtime-safe category-member shorthand first. Omit every unused optional key; do not pass empty strings such as \`teamName: ""\`, \`subagent_type: ""\`, or \`kind: ""\`. Lead ownership is always bound to the calling session and cannot be overridden.

Runtime-safe inline example:
  ...
  "members": [
    {
      ...
      "category": "quick",
      "prompt": "Analyze directory layouts, module boundaries, and architectural organization."
    },
    {
      "name": "quality-analyst",
      "category": "quick",
      "prompt": "Analyze tests, CI/CD, build scripts, conventions, and anti-patterns."
    }
  ]
}
\`\`\`

Declared TeamSpec files may use a lead plus members list. Canonical declared members have a \`kind\` discriminator.
...
Important: optional means **omit the key** when it is not used. Do not include unrelated member keys with empty-string values. For category members, omit \`subagent_type\`; for subagent members, omit \`category\` unless you are intentionally using the category shorthand.
...
- Omit \`correlationId\` in \`team_send_message\` unless you are copying an existing UUID. Do not invent human-readable slugs or task IDs.
