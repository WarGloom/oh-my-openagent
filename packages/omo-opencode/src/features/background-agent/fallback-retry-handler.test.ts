...
describe("tryFallbackRetry", () => {
  ...
  describe("#given retryable error with fallback chain", () => {
    ...
    test("aborts and awaits the existing session before the same-session callback and retry hook", async () => {
      // given
      const abortStarted = createDeferredPromise()
      const abortGate = createDeferredPromise()
      const callbackStarted = createDeferredPromise()
      const callOrder: string[] = []
      const sameSessionRetry = mock(async () => {
      ...
      args.abortMock.mockImplementationOnce(async () => {
      ...
      const firstCall = await Promise.race([
        abortStarted.promise.then(() => "abort" as const),
        callbackStarted.promise.then(() => "callback" as const),
      ])

      // then
      expect(firstCall).toBe("abort")
      ...
      expect(result).toBe(true)
      expect(callOrder).toEqual(["abort", "callback", "retry"])
    ...
    test("fails closed, releases the fallback slot, and preserves task state when same-session abort fails", async () => {
      // given
      ...
      const originalModel = args.task.model
      args.abortMock.mockImplementationOnce(async () => ({ error: "abort refused" }))
      ...
      expect(args.processKey).not.toHaveBeenCalled()
      expect(args.queuesByKey.size).toBe(0)
      expect(args.concurrencyManager.acquire).toHaveBeenCalledWith("provider-a/fallback-model-1")
      expect(args.concurrencyManager.release).toHaveBeenCalledTimes(1)
      expect(args.concurrencyManager.release).toHaveBeenCalledWith("provider-a/fallback-model-1")
      expect(args.task.model).toBe(originalModel)
