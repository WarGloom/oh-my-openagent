...
describe("team task tools", () => {
  ...
  test("team_task_update resolves subject slugs before updating", async () => {
    // given
    const config = createConfig()
    listTasksMock.mockImplementationOnce(async () => [{ id: "7", subject: "Fuzz Scope Auditor" } as Task])
    const updateTool = createTeamTaskUpdateTool(config, mockClient, deps)

    // when
    await updateTool.execute({ teamRunId: "team-run-1", taskId: "fuzz-scope-auditor", status: "in_progress" }, createContext("member-session-a"))

    // then
    expect(listTasksMock).toHaveBeenCalledWith("team-run-1", config)
    expect(updateTaskStatusMock).toHaveBeenCalledWith("team-run-1", "7", "in_progress", "member-a", config)
  })

  test("team_task_get resolves exact task subjects before loading", async () => {
    // given
    const config = createConfig()
    listTasksMock.mockImplementationOnce(async () => [{ id: "8", subject: "Exact Task" } as Task])
    const getTool = createTeamTaskGetTool(config, mockClient, deps)

    // when
    await getTool.execute({ teamRunId: "team-run-1", taskId: "Exact Task" }, createContext("member-session-a"))

    // then
    expect(listTasksMock).toHaveBeenCalledWith("team-run-1", config)
    expect(getTaskMock).toHaveBeenCalledWith("team-run-1", "8", config)
  })

  test("team_task_update reports missing non-numeric task references without path traversal errors", async () => {
    // given
    const config = createConfig()
    listTasksMock.mockImplementationOnce(async () => [])
    const updateTool = createTeamTaskUpdateTool(config, mockClient, deps)

    // when
    const result = updateTool.execute({ teamRunId: "team-run-1", taskId: "missing-task", status: "in_progress" }, createContext("member-session-a"))

    // then
    expect(result).rejects.toThrow("team task 'missing-task' not found")
  })

  test("team_task_update description does not advertise unsupported reassignment", () => {
    // given
    const config = createConfig()
    const updateTool = createTeamTaskUpdateTool(config, mockClient, deps)

    // then
    expect(updateTool.description).toContain("Claim, start, complete, or delete")
    expect(updateTool.description).toContain("Direct completion from pending/claimed auto-claims")
    expect(updateTool.description).not.toContain("reassign")
  })
