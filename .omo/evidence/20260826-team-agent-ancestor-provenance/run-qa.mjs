...
const rejectedScenarioDefinitions = new Map([
  ["rejected-source-shadow", {
    ...
    expectedErrorMessageFragment: "Project agent 'repository-reviewer' has no config-time provenance in the nearest registered snapshot for this directory or its ancestors.",
...
const evidenceSourceFiles = [
  ".omo/evidence/20260826-team-agent-ancestor-provenance/README.md",
  ".omo/evidence/20260826-team-agent-ancestor-provenance/fake-provider.mjs",
  ".omo/evidence/20260826-team-agent-ancestor-provenance/run-qa.mjs",
]
const intendedFiles = [
...
async function runScenario(scenario) {
  ...
  const omoConfigFile = path.join(project, ".omo", "omo.jsonc")
  ...
  try {
    for (const directory of [project, path.dirname(agentFile), path.dirname(omoConfigFile), configDir, environment.XDG_DATA_HOME, environment.XDG_STATE_HOME, environment.XDG_CACHE_HOME, environment.CODEX_HOME]) {
      mkdirSync(directory, { recursive: true })
    }
    exec("git", ["init", "--quiet"], { cwd: project, env: { ...environment, GIT_MASTER: "1" } })
    writeFileSync(agentFile, projectAgentMarkdown(scenario))
    writeFileSync(omoConfigFile, `${JSON.stringify({
      "[opencode]": {
        agents: {
...
const currentSourceStatusFiles = gitStatusShort()
  ...
  .filter((file) => !file.startsWith(".omo/evidence/20260826-team-agent-ancestor-provenance/"))
  .filter((file) => !file.endsWith("20260816-remove-omo-telemetry-command/disclosure-tui/terminal-ansi.txt"))
  .filter((file) => !file.endsWith("20260816-remove-omo-telemetry-command/tui-pass/terminal-ansi.txt"))
  .sort()
const sourceManifestMatchesStatus = currentSourceStatusFiles.length === 0
...
const assertions = accepted && rejectedScenarios.every((scenario) => rejected.get(scenario) !== undefined) ? {
  exactSourceManifestHas16Files: sourceManifest.length === 16,
  sourceTreeCleanAtQaHead: sourceManifestMatchesStatus,
