    ...
    if (availableModels.size === 0) {
      ...
      if (connectedSet !== null) {
        for (const model of userFallbackModels) {
          ...
          const transformedModel = `${firstProvider}/${transformModelForProvider(firstProvider, parsedFallback.baseModel)}`
      ...
      for (const model of userFallbackModels) {
        ...
        const match = fuzzyMatchModel(parsedFallback.baseModel, availableModels, providerHint)
        if (match) {
